...
pub fn test_column_family() {
    let temp_dir = TempDir::new("_rust_rocksdb_cftest").unwrap();
...
fn test_merge_operator() {
    let temp_dir = TempDir::new("_rust_rocksdb_cftest_merge").unwrap();
    let path = temp_dir.path();
    // TODO should be able to write, read, merge, batch, and iterate over a cf
    {
        ...
        assert_eq!(
            db.get_cf(cf1, b"k1").unwrap().unwrap().to_utf8().unwrap(),
            "v1"
        );
